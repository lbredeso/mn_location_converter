...
5. Import the events, where [file] is relative to lib/data/location.  Some samples are included here:  https://github.com/lbredeso/mn_location_converter/downloads, for crashes as defined in DPS data.

    rake events:import[file]
    
6. Download the shapefiles

    rake download_shapefiles
    
7. Generate the roads files

    rake generate_roads
    
8. Import the roads files

    sudo su - postgres
    # cd to root of this repository
    for f in lib/data/roads/*.sql ; do psql -d mn_location_converter_development -f $f ; done
    
9. Calculate and save latitude and longitude values

    rake calculate_lat_lon
    
10. Export the events to a file, where [file] is the name of the file, relative to the current directory.
